...
{JSON.stringify(docResult, null, 2)}
        ...
        {/* ──────────────────────────────────────────────────────────────────────────
           ...
           ────────────────────────────────────────────────────────────────────────── */}
        <section className="aitools-section aitools-bg-darker" id="use-cases">
          <div className="aitools-container">
            
            <div className="aitools-section-header">
              <span className="aitools-eyebrow text-green">
                <i className="fas fa-industry"></i> SECTOR VERTICALS
              </span>
              <h2 className="aitools-section-title">
                AI Solutions Customized by <span className="aitools-gradient-cyan">Industry</span>
              </h2>
              <p className="aitools-section-subtitle">
            ...
            {/* Industry Switcher */}
            <div className="aitools-pills-bar">
              <button
                className={`aitools-pill ${activeIndustry === 'ecommerce' ? 'active' : ''}`}
                onClick={() => setActiveIndustry('ecommerce')}
              >
                🛒 E-Commerce & Retail
              </button>
              <button
                className={`aitools-pill ${activeIndustry === 'healthcare' ? 'active' : ''}`}
                onClick={() => setActiveIndustry('healthcare')}
              >
                🏥 Healthcare & Clinics
              </button>
              <button
                className={`aitools-pill ${activeIndustry === 'finance' ? 'active' : ''}`}
                onClick={() => setActiveIndustry('finance')}
              >
                💰 Finance & Banking
              </button>
              <button
                className={`aitools-pill ${activeIndustry === 'restaurant' ? 'active' : ''}`}
                onClick={() => setActiveIndustry('restaurant')}
              >
                🍽️ Hospitality & Food
              </button>
              <button
                className={`aitools-pill ${activeIndustry === 'saas' ? 'active' : ''}`}
                onClick={() => setActiveIndustry('saas')}
              >
                ⚙️ SaaS & Tech
              </button>
              <button
                className={`aitools-pill ${activeIndustry === 'realestate' ? 'active' : ''}`}
            ...
            <div className="aitools-cards-grid">
              {activeIndustry === 'ecommerce' && (
                <>
                  <div className="aitools-card aitools-usecase-card">
                    <div className="emoji-icon">🤖</div>
                    <h4>AI Support Chatbot</h4>
                    <p>Auto-resolves shipping, order status, and return requests with live Shopify sync.</p>
                    <span className="roi-badge text-cyan">ROI: -65% Support Costs</span>
                  </div>
                  <div className="aitools-card aitools-usecase-card">
                    <div className="emoji-icon">📦</div>
                    <h4>Inventory Demand Forecasting</h4>
                    <p>Predict stock requirements, auto-trigger reorders, and prevent stockouts during peak sales.</p>
                    <span className="roi-badge text-green">ROI: -35% Holding Costs</span>
                  </div>
                  <div className="aitools-card aitools-usecase-card">
                    <div className="emoji-icon">✉️</div>
                    <h4>AI Abandoned Cart Flow</h4>
                    <p>Hyper-personalized email sequences with send-time prediction and AI subject lines.</p>
              ...
              {activeIndustry === 'healthcare' && (
                <>
                  ...
                  <div className="aitools-card aitools-usecase-card">
                    ...
                    <p>Automated appointment scheduling and SMS reminders to eliminate patient no-shows.</p>
                    <span className="roi-badge text-green">ROI: -60% No-Shows</span>
                  </div>
                  <div className="aitools-card aitools-usecase-card">
                    <div className="emoji-icon">🧠</div>
                    <h4>Clinical RAG Knowledge Base</h4>
                    <p>Instant protocol lookups for doctors and nursing staff with exact source page citations.</p>
              ...
              {activeIndustry === 'finance' && (
                <>
                  <div className="aitools-card aitools-usecase-card">
                    ...
                    <p>Real-time transaction fraud scoring and credit risk analysis on 50+ financial signals.</p>
              ...
              {activeIndustry === 'restaurant' && (
                <>
                  <div className="aitools-card aitools-usecase-card">
                    ...
                    <p>Inbound AI voice agent answers phone calls, books tables, and manages waitlists 24/7.</p>
              ...
              {activeIndustry === 'realestate' && (
                <>
                  ...
                  <div className="aitools-card aitools-usecase-card">
                    ...
                    <p>Extract tenant terms, rent amounts, and expiration dates into property software.</p>
